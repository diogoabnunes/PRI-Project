...
constructorIds = constructors['constructorId']
urls = constructors['url']

data = {'constructorId': [], 'text': []}

for i in range(len(constructors)):
    constructorId = constructorIds[i]
    print(f'constructorID: {constructorId}')
    url = urls[i]
    ...
    while (first_p is not None and first_p != last):
        if (first_p.name == 'p'):
            ...
            if text and text != '(key)':
                data['constructorId'].append(constructorId)
                data['text'].append(text) 
        first_p = first_p.find_next_sibling()

df = pd.DataFrame(data=data)
df.to_csv('../processed_data/constructors_text.csv', index=False)